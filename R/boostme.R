...
i=sample_index
    ...
    if (impute) {
     ...
     write.table(metrics, file = paste0(colnames(bs)[sample_index], "metrics.txt"), quote = F, sep = "\t", row.names = F, col.names = F)
     write(imputed, file = paste0(colnames(bs)[sample_index], "imputed.txt"), ncolumns=1)
  if (verbose) {
